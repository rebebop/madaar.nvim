local theme = {}
local editor_group = require("madaar.groups.editor_group")
local syntax_group = require("madaar.groups.syntax_group")
...
function theme.highlights(colors, config)
	...
	local function load_syntax()
		...
		return syntax_group.highlights(colors, config)
	end

	local function load_editor()
		-- Editor highlight groups

		local editor = editor_group.highlights(colors, config)
		-- Options:

		-- Set non-current background
		if config.fade_nc then
			editor.NormalNC["bg"] = colors.fg1
			editor.NormalFloat["bg"] = colors.bg0
			editor.FloatBorder["bg"] = colors.bg0
		end
		...
		-- 	editor.EndOfBuffer["fg"] = colors.bg0
		-- end
